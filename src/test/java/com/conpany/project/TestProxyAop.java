 ...
 *
 * 代理模式:
 * 代理设计模式的一个很好的例子是org.springframework.aop.framework.ProxyFactoryBean。
 * 该工厂根据Spring bean构建AOP代理。该类实现了定义getObject()方法的 FactoryBean接口。
 * 此方法用于将需求 Bean的实例返回给 bean factory。在这种情况下，它不是返回的实例，而是 AOP代理。
 **/
